...
class IU_UltimateManager:
    def __init__(self, root):
        self.root = root
        self.app_version = "0.28v"  # 修改需求3：版本号递增
        ...
        self.tasks = [] 
        self.task_cb_vars = {} 
        self.alarm_countdown_labels = [] # 新增：存放闹钟倒计时Label的引用
            ...
            for r_idx, week in enumerate(cal_matrix):
                for c_idx, day in enumerate(week):
                    if day != 0:
                        ...
                        btn = tk.Button(days_f, text=str(day), font=(self.ui_font, 10), bg=bg_color, fg=fg_color,
    ...
    def render_task_page(self):
        ...
        # 修改需求2：用原生自制日历下拉替代Combobox，保持UI高度一致
            ...
            for label, a in self.alarm_countdown_labels:
                if label.winfo_exists() and a['active']:
                    try:
                        ...
                        if alarm_dt <= now:
                            # 按照全局监控的逻辑，只要还在active且时间已过，实质是等明天触发（或是漏了即将在下一秒触发）
    ...
    def render_alarm_list(self):
        ...
        self.alarm_countdown_labels = []  # 修改需求1：清空存放倒计时标签的列表
