...
class IU_UltimateManager:
    def __init__(self, root):
        self.root = root
        self.app_version = "0.30v"  # 修改需求：版本号递增为 0.30v
    ...
    # ================= 原生手写UI风格日历下拉组件 =================
    def open_calendar_dropdown(self, widget, text_var, callback=None):
        if hasattr(self, '_cal_top') and self._cal_top.winfo_exists():
        ...
        top.configure(bg="white", highlightbackground="#bdc3c7", highlightthickness=1)

        x = widget.winfo_rootx()
        y = widget.winfo_rooty() + widget.winfo_height() + 2
        top.geometry(f"+{x}+{y}")

        curr_date = datetime.strptime(text_var.get(), "%Y-%m-%d") if text_var.get() else datetime.now()
        sel_year = tk.IntVar(value=curr_date.year)
        sel_month = tk.IntVar(value=curr_date.month)

        cal_frame = tk.Frame(top, bg="white", padx=10, pady=10)
        cal_frame.pack()

        header_f = tk.Frame(cal_frame, bg="white")
        header_f.pack(fill=tk.X, pady=(0, 10))
        
        dates_with_data = set(t.get('date', '') for t in self.tasks if t.get('date'))

        def update_cal():
            for w in days_f.winfo_children(): w.destroy()
            y_val, m_val = sel_year.get(), sel_month.get()
            month_lbl.config(text=f"{y_val}年 {m_val}月")

            weekdays = ["一", "二", "三", "四", "五", "六", "日"]
            for i, wd in enumerate(weekdays):
                tk.Label(days_f, text=wd, font=(self.ui_font, 9, "bold"), bg="white", fg="#7f8c8d", width=4).grid(row=0, column=i, pady=2)

            cal_matrix = calendar.monthcalendar(y_val, m_val)
            today_str = time.strftime("%Y-%m-%d")
            for r_idx, week in enumerate(cal_matrix):
                for c_idx, day in enumerate(week):
                    if day != 0:
                        date_str = f"{y_val}-{m_val:02d}-{day:02d}"
                        
                        if date_str == text_var.get():
                            bg_color = "#1abc9c"
                            fg_color = "white"
                        elif date_str in dates_with_data:
                            bg_color = "#d1f2eb"  
                            fg_color = "#16a085"  
                            if date_str == today_str: fg_color = "#e74c3c"
                        else:
                            bg_color = "white"
                            fg_color = "#2c3e50"
                            if date_str == today_str: fg_color = "#e74c3c"

                        btn = tk.Button(days_f, text=str(day), font=(self.ui_font, 10, "bold" if date_str in dates_with_data else "normal"), 
                                        bg=bg_color, fg=fg_color,
                                        bd=0, activebackground="#1abc9c", activeforeground="white", cursor="hand2", width=3,
        ...
        def change_month(delta):
            ...
            sel_month.set(m); sel_year.set(y)
            update_cal()

        tk.Button(header_f, text="◀", bg="white", fg="#7f8c8d", bd=0, cursor="hand2", command=lambda: change_month(-1)).pack(side=tk.LEFT)
        month_lbl = tk.Label(header_f, font=(self.ui_font, 11, "bold"), bg="white", fg="#2c3e50")
        month_lbl.pack(side=tk.LEFT, expand=True)
        tk.Button(header_f, text="▶", bg="white", fg="#7f8c8d", bd=0, cursor="hand2", command=lambda: change_month(1)).pack(side=tk.RIGHT)

        days_f = tk.Frame(cal_frame, bg="white")
        days_f.pack()

        btn_f = tk.Frame(cal_frame, bg="white")
        btn_f.pack(fill=tk.X, pady=(10, 0))
        tk.Button(btn_f, text="取消/关闭", font=(self.ui_font, 9), bg="#ecf0f1", fg="#7f8c8d", bd=0, cursor="hand2", command=top.destroy).pack(fill=tk.X)
    ...
    def render_task_page(self):
        tab_frame = tk.Frame(self.main_container, bg="white")
        tab_frame.pack(fill=tk.X, pady=(0, 10))

        def switch_tab(mode):
            self.task_tab_mode = mode
            self.show_page("任务")

        add_bg = "#1abc9c" if self.task_tab_mode == "添加任务" else "#ecf0f1"
        add_fg = "white" if self.task_tab_mode == "添加任务" else "#7f8c8d"
        add_font = (self.ui_font, 11, "bold" if self.task_tab_mode == "添加任务" else "normal")

        all_bg = "#1abc9c" if self.task_tab_mode == "查看所有任务" else "#ecf0f1"
        all_fg = "white" if self.task_tab_mode == "查看所有任务" else "#7f8c8d"
        all_font = (self.ui_font, 11, "bold" if self.task_tab_mode == "查看所有任务" else "normal")

        tk.Button(tab_frame, text="添加任务", font=add_font, bg=add_bg, fg=add_fg, bd=0, padx=20, pady=6, cursor="hand2", 
                  command=lambda: switch_tab("添加任务")).pack(side=tk.LEFT, padx=(0, 10))
        
        tk.Button(tab_frame, text="查看所有任务", font=all_font, bg=all_bg, fg=all_fg, bd=0, padx=20, pady=6, cursor="hand2", 
                  command=lambda: switch_tab("查看所有任务")).pack(side=tk.LEFT)

        ttk.Separator(self.main_container, orient='horizontal').pack(fill=tk.X, pady=5)

        if self.task_tab_mode == "添加任务":
            self.render_add_task_view()
        else:
            self.render_all_tasks_view()

    def render_add_task_view(self):
        ...
        self.date_btn = tk.Button(filter_frame, textvariable=self.current_view_date,
                                  font=(self.ui_font, 11), bg="white", fg="#2c3e50",
                                  bd=1, relief="solid", width=12, cursor="hand2",
                                  command=lambda: self.open_calendar_dropdown(self.date_btn, self.current_view_date, self.refresh_task_list))
        self.date_btn.pack(side=tk.LEFT, padx=10)
        ...
        self.task_hint_label = tk.Label(self.main_container, text="", bg="white", fg="#7f8c8d", font=(self.ui_font, 9))
    ...
    def render_all_tasks_view(self):
        # 新增需求1：展开/折叠控制区域
        ctrl_frame = tk.Frame(self.main_container, bg="white")
        ctrl_frame.pack(fill=tk.X, pady=(0, 5))
        
        self.toggle_callbacks = [] # 存储所有折叠控件的回调函数
        
        def expand_all():
            for toggle in self.toggle_callbacks: toggle(True)
            
        def collapse_all():
            for toggle in self.toggle_callbacks: toggle(False)

        tk.Button(ctrl_frame, text="▼ 展开所有任务", font=(self.ui_font, 9), bg="#ecf0f1", fg="#2c3e50", bd=0, padx=12, pady=4, cursor="hand2", command=expand_all).pack(side=tk.LEFT, padx=(0, 8))
        tk.Button(ctrl_frame, text="▶ 折叠所有任务", font=(self.ui_font, 9), bg="#ecf0f1", fg="#2c3e50", bd=0, padx=12, pady=4, cursor="hand2", command=collapse_all).pack(side=tk.LEFT)
        ...
        all_frame = tk.Frame(list_canvas, bg="white")
        all_frame.bind("<Configure>", lambda e: list_canvas.configure(scrollregion=list_canvas.bbox("all")))
        
        _win_id = list_canvas.create_window((0, 0), window=all_frame, anchor="nw")
        ...
        list_canvas.bind("<Enter>", lambda e: list_canvas.bind_all("<MouseWheel>", lambda ev: list_canvas.yview_scroll(int(-1*(ev.delta/120)), "units")))
        ...
        if not self.tasks:
            tk.Label(all_frame, text="暂无任何历史任务记录。", bg="white", font=(self.ui_font, 12), fg="#7f8c8d").pack(pady=30)
            return
        ...
        for t in self.tasks:
            ...
            try:
                ...
                grouped_tasks[dt.year][dt.month].append((dt.day, t))
            except Exception:
                pass

        # 辅助函数：创建折叠控制器
        def create_toggler(btn, content_frame, text_base):
            is_expanded = [True] # 默认展开
            def toggle(force_state=None):
                if force_state is not None:
                    if is_expanded[0] == force_state: return
                    is_expanded[0] = force_state
                else:
                    is_expanded[0] = not is_expanded[0]
                    
                if is_expanded[0]:
                    content_frame.pack(fill=tk.X)
                    btn.config(text=f"▼ {text_base}")
                else:
                    content_frame.pack_forget()
                    btn.config(text=f"▶ {text_base}")
            btn.config(command=toggle)
            self.toggle_callbacks.append(toggle)
            return toggle

        # 新增需求2：年、月、日 分级折叠渲染
        for year in sorted(grouped_tasks.keys(), reverse=True):
            y_frame = tk.Frame(all_frame, bg="white")
            y_frame.pack(fill=tk.X, pady=(10, 0))
            
            y_btn = tk.Button(y_frame, text=f"▼ {year} 年", font=(self.ui_font, 14, "bold"), bg="white", fg="#2c3e50", bd=0, cursor="hand2", anchor="w")
            y_btn.pack(fill=tk.X, padx=10)
            
            y_content = tk.Frame(y_frame, bg="white")
            y_content.pack(fill=tk.X)
            create_toggler(y_btn, y_content, f"{year} 年")

            for month in sorted(grouped_tasks[year].keys(), reverse=True):
                m_frame = tk.Frame(y_content, bg="white")
                m_frame.pack(fill=tk.X, pady=(2, 0))
                
                m_btn = tk.Button(m_frame, text=f"▼ {month} 月", font=(self.ui_font, 12, "bold"), bg="white", fg="#34495e", bd=0, cursor="hand2", anchor="w")
                m_btn.pack(fill=tk.X, padx=25)
                
                m_content = tk.Frame(m_frame, bg="white")
                m_content.pack(fill=tk.X)
                create_toggler(m_btn, m_content, f"{month} 月")
                
                day_tasks_dict = defaultdict(list)
                for day, task in grouped_tasks[year][month]:
                    day_tasks_dict[day].append(task)
                    
                for day in sorted(day_tasks_dict.keys(), reverse=True):
                    d_wrapper = tk.Frame(m_content, bg="#f8f9fa", pady=5, padx=15)
                    d_wrapper.pack(fill=tk.X, padx=40, pady=3)
                    
                    d_btn = tk.Button(d_wrapper, text=f"▼ {day} 日", font=(self.ui_font, 11, "bold"), bg="#f8f9fa", fg="#7f8c8d", bd=0, cursor="hand2", anchor="w")
                    d_btn.pack(fill=tk.X)
                    
                    d_content = tk.Frame(d_wrapper, bg="#f8f9fa")
                    d_content.pack(fill=tk.X)
                    create_toggler(d_btn, d_content, f"{day} 日")
                    
                    for task in day_tasks_dict[day]:
                        status_icon = "✅" if task.get('completed') else "⬜"
                        fg_color = "#7f8c8d" if task.get('completed') else "#2c3e50"
                        f_style = "overstrike" if task.get('completed') else "normal"
                        tk.Label(d_content, text=f"{status_icon} {task['text']}", font=(self.ui_font, 10, f_style), bg="#f8f9fa", fg=fg_color).pack(anchor="w", padx=20, pady=2)
    ...
    def edit_task(self, task):
        ...
        edit_date_btn = tk.Button(date_f, textvariable=date_var,
                                  font=(self.ui_font, 10), bg="white", fg="#2c3e50",
                                  bd=1, relief="solid", width=12, cursor="hand2",
                                  command=lambda: self.open_calendar_dropdown(edit_date_btn, date_var))
        edit_date_btn.pack(side=tk.LEFT)
    ...
    def refresh_task_list(self):
        ...
        if not filtered_tasks:
            msg = "今日暂无任务，王总请即刻添加学习计划！" if view_date == today_str else f"{view_date} 无记录，可在上方日历选择或输入框添加任务。"
    ...
    def update_live_clock(self):
        ...
        if hasattr(self, 'alarm_countdown_labels'):
            now = datetime.now()
            for label, a in self.alarm_countdown_labels:
                if label.winfo_exists() and a['active']:
                    try:
                        alarm_dt = datetime.strptime(a['time'], "%H:%M:%S").replace(year=now.year, month=now.month, day=now.day)
                        if alarm_dt <= now:
                        ...
                        if diff > 0:
                            ...
                            label.config(text=f"⏳ 倒计时 {int(hours)}小时{int(mins)}分钟后响铃")
                        else:
                            label.config(text="🔔 即将响铃")
                    except Exception:
                        pass
    ...
    def render_alarm_list(self):
        ...
        for a in self.active_alarms:
            ...
            tk.Label(row, text=display_str, bg="#f8f9fa", fg=font_col, font=(self.big_font, 11)).pack(side=tk.LEFT, padx=(25, 10))
            
            if a['active']:
                cd_label = tk.Label(row, text="", bg="#f8f9fa", fg="#e67e22", font=(self.ui_font, 10, "bold"))
                cd_label.pack(side=tk.LEFT, padx=5)
                self.alarm_countdown_labels.append((cd_label, a))
